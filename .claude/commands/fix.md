...
Antimetal generates remediation from its infrastructure-level view. File names, paths, and patterns may not match the local codebase exactly.

### Code Steps

1. **Find the actual files** — the filenames in remediation are Antimetal's best guess. Search the local codebase for the real targets.

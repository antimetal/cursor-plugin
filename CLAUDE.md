...
Antimetal's software investigation intelligence plugin. Triage problems, investigate root causes, fetch observability artifacts, and apply remediations.

## Conventions

- **API key required**: All MCP tools need a valid `ANTIMETAL_API_KEY` environment variable. If a tool call fails with an auth error, prompt the user to set it: `export ANTIMETAL_API_KEY="your-key"` (get one at [antimetal.com](https://antimetal.com) under account settings). Suggest adding it to their shell profile so it persists.
...
- **info**: Context and explanation for why the fix works
- **code**: File changes as `{ filename, language?, content }` — adapt to local codebase
- **cli**: Shell commands — adapt for local environment, present for user approval
